...
namespace Free.Ports.Proj4
{
	public static partial class Proj
	{
		...
		static readonly string[] pj_err_list=new string[]
		{
		...
		public static string pj_strerrno(int err)
		{
			if(err>=0) return Libc.strerror(err);

			int adjusted_err=-err-1;
			if(adjusted_err<pj_err_list.Length) return pj_err_list[adjusted_err];
			return string.Format("invalid projection system error ({0})", err);
		}
	}
}
